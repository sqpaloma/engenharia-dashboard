...
const defaultUsers: User[] = [
  { id: "1", username: "admin", name: "Administrador", role: "admin", isOnline: false },
  { id: "2", username: "paloma", name: "Paloma", role: "engineer", isOnline: false },
  { id: "3", username: "giovanni", name: "Giovanni", role: "engineer", isOnline: false },
  { id: "4", username: "lucas", name: "Lucas", role: "engineer", isOnline: false },
  { id: "5", username: "marcelo", name: "Marcelo", role: "engineer", isOnline: false },
  { id: "6", username: "consultor1", name: "Consultor 1", role: "user", isOnline: false },
  { id: "7", username: "consultor2", name: "Consultor 2", role: "user", isOnline: false },
]
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateUserOnlineStatus = (userId: string, isOnline: boolean) => {
    setUsers((prevUsers) => {
      const updatedUsers = prevUsers.map((u) =>
        u.id === userId ? { ...u, isOnline, lastSeen: isOnline ? new Date() : u.lastSeen } : u,
      )

      // Salvar usuários online
      const onlineUserIds = updatedUsers.filter((u) => u.isOnline).map((u) => u.id)
  ...
  const login = async (username: string, password: string): Promise<boolean> => {
    ...
    if (foundUser && userCredentials[username] === password) {
      const userWithOnline = { ...foundUser, isOnline: true, lastSeen: new Date() }
