    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-600">Redirecionando para seu departamento...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-4">Bem-vindo, {user?.name}!</h1>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto">
            Escolha o departamento para visualizar o dashboard específico com dados operacionais e métricas de
            desempenho
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {departamentos.map((dept) => {
            ...
            return (
              <Card key={dept.id} className="hover:shadow-lg transition-all duration-300 hover:scale-105">
                <CardHeader className="text-center">
                  ...
                  <CardDescription className="text-base">
                    Responsáveis: <span className="font-medium">{dept.responsaveis.join(", ")}</span>
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Button asChild className="w-full" size="lg">
                    <Link href={`/dashboard?setor=${dept.id}`}>Acessar Dashboard</Link>
                  </Button>
                </CardContent>
              </Card>
            )
        ...
        <div className="text-center mt-12">
          <p className="text-slate-500">
            ...
            para se comunicar com todos os membros
          </p>
        </div>
      </div>
    </div>
